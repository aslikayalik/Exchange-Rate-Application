   ...
   public class BusinessLogicLayer:BaseClass
    {
        ...
        public DataTable KurGecmisGoruntule()
        {
            ...
            List<ParaBirimi> ParaBirimiList = ParaBirimiListesi();
            for (int i = 0; i< KurGecmisList.Count; i++) 
            {
                DT.Rows.Add(
                    ParaBirimiList.FirstOrDefault(I => I.ID == KurGecmisList[i].ParaBirimiID).Tanim, 
                    ParaBirimiList.FirstOrDefault(I => I.ID == KurGecmisList[i].ParaBirimiID).Code,
    ...
    private void EmailGonder(JsonDataType kurBilgisi, ParaBirimi paraBirimi)
        {
            ...
            MailAddress From = new MailAddress("doviz@kayalikasli.com","Döviz Uygulaması",encode); 
            MailAddress TO = new MailAddress("kayalik.asli@gmail.com", "Aslı Kayalık", encode);  
            Email.To.Add(TO);
            Email.From = From;
            ...
            string SMTPServer = "mail.kayalikasli.com";
            int SMTPPort = 587;
            string KullaniciAdi = "doviz@kayalikasli.com";
            string Sifre = "lkA0hgfx1";
